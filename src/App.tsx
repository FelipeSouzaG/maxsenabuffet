...
type ClientModalTab = "data" | "address";
...
export default function App() {
  ...
  const [clientModalTab, setClientModalTab] = useState<ClientModalTab>("data");
  ...
  const [quoteProgressText, setQuoteProgressText] = useState("");
  const [contractQuote, setContractQuote] = useState<Quote | null>(null);
  const [contractDraft, setContractDraft] = useState<QuoteContract | null>(null);
  const [contractSaving, setContractSaving] = useState(false);
  const [contractProgressText, setContractProgressText] = useState("");
  ...
  const openClientModal = (mode: ClientModalMode, client?: Client) => {
    ...
    setClientForm({
      ...
      notes: client.notes || "",
      addressCep: client.addressCep ? formatCep(client.addressCep) : "",
  ...
  const loadClientCep = async () => {
    const cepDigits = onlyDigits(clientForm.addressCep);
    ...
    if (!result.ok) {
      setClientCepError(result.error);
    ...
    setClientForm((prev) => ({
      ...
      addressDistrict: result.address.district || prev.addressDistrict,
  ...
  const filteredClientOptions = useMemo(() => {
    ...
    return clients
      .filter((client) => {
        ...
        return (
          textFields.includes(q) || (!!digits && digitFields.includes(digits))
        );
      })
      .slice(0, 12);
  ...
  const saveClient = async (event: FormEvent) => {
    ...
    const payload = {
      ...
      addressCep: clientForm.addressCep ? onlyDigits(clientForm.addressCep) : "",
      addressStreet: clientForm.addressStreet.trim(),
      addressNumber: clientForm.addressNumber.trim(),
      addressComplement: clientForm.addressComplement.trim(),
      addressDistrict: clientForm.addressDistrict.trim(),
      addressCity: clientForm.addressCity.trim(),
      addressState: clientForm.addressState.trim().toUpperCase(),
    };
  ...
  const loadContractDraft = async (quote: Quote, regenerate = false) => {
    const contract = await api<QuoteContract>(
      `/api/quotes/${quote.id}/contract/draft`,
      {
        method: "POST",
  ...
  const openContractReview = async (quote: Quote, regenerate = false) => {
    setContractSaving(true);
    setContractProgressText("Gerando Contrato");
    setContractError("");
    try {
      await loadContractDraft(quote, regenerate);
    } catch (err: any) {
      setContractError(err?.message || "Falha ao abrir contrato.");
      alert(err?.message || "Falha ao abrir contrato.");
    } finally {
      setContractSaving(false);
      setContractProgressText("");
    }
  ...
  const saveContractDraft = async () => {
    if (!contractDraft || !contractQuote) return;
    setContractSaving(true);
    setContractProgressText("Salvando dados de Contrato");
    setContractError("");
    try {
      await api<QuoteContract>(
        `/api/quotes/${contractQuote.id}/contract`,
        {
          method: "PATCH",
          body: JSON.stringify({ sections: contractDraft.sections }),
        },
      );
      await refreshAll();
      closeContractModal();
    } catch (err: any) {
      setContractError(err?.message || "Falha ao salvar contrato.");
    } finally {
      setContractSaving(false);
      setContractProgressText("");
    }
  ...
  const issueContract = async () => {
    ...
    setContractError("");
    try {
      const issued = await api<QuoteContract>(
        `/api/quotes/${contractQuote.id}/contract/issue`,
        {
          method: "POST",
          body: JSON.stringify({ sections: contractDraft.sections }),
        },
      );
      setContractDraft(issued);
      setContractProgressText("Gerando PDF");
      await generateContractPDF({ contract: issued, quote: contractQuote });
      await refreshAll();
      closeContractModal();
    } catch (err: any) {
      setContractError(err?.message || "Falha ao emitir contrato.");
    } finally {
      setContractSaving(false);
      setContractProgressText("");
    }
  ...
  const loadCep = async () => {
    ...
    setQuoteForm((prev) => ({
      ...prev,
      eventLocationStreet: result.address.street || prev.eventLocationStreet,
      eventLocationDistrict: result.address.district || prev.eventLocationDistrict,
      eventLocationCity: result.address.city || prev.eventLocationCity,
      eventLocationState: result.address.state || prev.eventLocationState,
    }));
    setCepLoading(false);
  };
  ...
  const saveQuote = async (event: FormEvent) => {
    ...
    const payload = {
      ...
      totalValue,
      paymentConditions: quoteForm.paymentConditions || null,
    ...
    try {
      setQuoteSaving(true);
      setQuoteProgressText("Salvando dados de Orçamento");

      if (quoteModalMode === "edit" && quoteForm.id) {
        savedQuote = await api<Quote>(`/api/quotes/${quoteForm.id}`, {
          method: "PATCH",
          body: JSON.stringify(payload),
        });
      ...
      if (savedQuote?.status === "APPROVED") {
        setQuoteProgressText("Gerando Evento");
        await new Promise<void>((resolve) => window.setTimeout(resolve, 150));
        setQuoteProgressText("Gerando Contrato");
        await loadContractDraft(savedQuote);
      }
  ...
  const deleteQuote = async (quoteId: string) => {
    ...
    closeQuoteModal();
    if (contractQuote?.id === quoteId) closeContractModal();
    await refreshAll();
  };
                        ...
                        <td data-label="Ações">
                          <div className="actions">
                            ...
                            {quote.status === "APPROVED" ? (
                              <button
                                type="button"
                                onClick={() => openContractReview(quote)}
                              >
                                Contrato
                              </button>
                            ) : null}
        ...
        <div className="modal-overlay">
          <div className="modal-card modal-card-sm">
            ...
            <div className="modal-tabs">
              <button
                type="button"
                className={clientModalTab === "data" ? "active" : ""}
                onClick={() => setClientModalTab("data")}
              >
                Dados do cliente
              </button>
              <button
                type="button"
                className={clientModalTab === "address" ? "active" : ""}
                onClick={() => setClientModalTab("address")}
              >
                Endereço
              </button>
            </div>
              ...
              {clientModalTab === "data" ? (
                ...
                <>
                  <p className="muted small-hint">
                    Consulte o CEP para preencher logradouro, bairro, cidade e UF. Os campos
                    permanecem editáveis para correções quando a base do ViaCEP estiver imprecisa.
                  </p>
                  ...
                  <div className="row row-2">
                    <label>
                      CEP
                      <input
                        value={clientForm.addressCep}
                        maxLength={10}
                        onChange={(event) =>
                          setClientForm((prev) => ({
                            ...prev,
                            addressCep: formatCep(event.target.value),
                          }))
                        }
                        onBlur={loadClientCep}
                  ...
                  {clientCepLoading ? <small>Consultando CEP...</small> : null}
                  {clientCepError ? <small className="error">{clientCepError}</small> : null}
                  ...
                  <div className="row">
                    <label>
                      Bairro
                      <input
                        value={clientForm.addressDistrict}
                        onChange={(event) =>
                          setClientForm((prev) => ({
                            ...prev,
      ...
      {quoteModalMode ? (
        <div className="modal-overlay">
          <div className="modal-card">
            ...
            {quoteSaving && quoteProgressText ? (
              <div className="loading-status">
                <span className="loading-spinner" />
                <span>{quoteProgressText}</span>
              </div>
            ) : null}
            ...
            <form className="modal-form" onSubmit={saveQuote}>
              ...
              <label>
                ...
                <textarea
                  value={quoteForm.paymentConditions}
                  onChange={(event) =>
                    setQuoteForm((prev) => ({
                      ...prev,
                  ...
                  placeholder="Ex: R$ 2.000,00 de entrada e restante até o dia do evento."
                  disabled={quoteModalMode === "view"}
                />
              </label>
              ...
              <div className="modal-actions">
                ...
                {quoteModalMode !== "view" ? (
                  <button
                    type="submit"
                    disabled={!quoteForm.status || !quoteRules.length || quoteSaving}
                  >
                    {quoteSaving ? (
                      <>
                        <span className="loading-spinner is-light" />
                        {quoteProgressText || submitLabel}
                      </>
                    ) : (
                      submitLabel
                    )}
      ...
      {contractDraft && contractQuote ? (
        <div className="modal-overlay">
          <div className="modal-card contract-modal">
            <header className="modal-header">
              <div>
                ...
                <p className="small-hint">
                  {contractQuote.number} • {contractQuote.client?.name || "Cliente"}
                </p>
              </div>
              <button
                type="button"
            ...
            {contractError ? (
              <div className="error-box">{contractError}</div>
            ) : null}

            {contractSaving && contractProgressText ? (
              <div className="loading-status">
                <span className="loading-spinner" />
            ...
            <div className="contract-review">
              {contractDraft.sections.map((section, index) => (
                <section className="contract-section-editor" key={section.id}>
            ...
            <div className="modal-actions">
              ...
              <button
                type="button"
                className="btn-ghost"
                onClick={() => openContractReview(contractQuote, true)}
                disabled={contractSaving}
              >
                Recarregar padrão
              </button>
              ...
              >
                {contractSaving ? (
                  <>
                    <span className="loading-spinner is-light" />
                    {contractProgressText || "Gerando PDF"}
                  </>
                ) : (
                  "Salvar e gerar PDF"
                )}
